...
- [ ] 每条消息的字段级编码（13B / 48B 布局、各字段位定义）
- [ ] MIC 计算（PLOAM-IK 派生、CMAC）
- [ ] SeqNum / Ack 重传机制（G.989.3 §11.3.5）
- [ ] GPON 13B 与 XGS-PON 48B 的逐字段对照

## 来源

- **工程实现**：`gopon/common/ploam/ids.go`（DsMsgID / UsMsgID 全集 + String()）、`gopon/common/ploam/frame.go`（Frame 结构、SeqNum、Direction）。
- **公有标准（待检索补全字段）**：ITU-T G.984.3 §9.2（GPON PLOAM）、G.9807.1 §C.11 / G.989.3 §11（48B PLOAM 消息格式）。
